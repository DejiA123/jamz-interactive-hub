...
function StudioPage() {
  ...
  const [draft, setDraft] = useState<{ kind: Kind; prompt: string; duration: number; points: number; options: string[] }>({ kind: "poll", prompt: "", duration: 45, points: 0, options: ["Not guilty", "Guilty"] });
  ...
  function openTemplate(template?: Template) {
    setDraft(template ? { kind: template.kind, prompt: template.prompt, duration: 45, points: template.kind === "quiz" ? 1000 : 0, options: template.options ?? [] } : { kind: "poll", prompt: "", duration: 45, points: 0, options: ["Option one", "Option two"] });
  ...
  async function saveActivity(event: React.FormEvent) {
    ...
    if ((draft.kind === "poll" || draft.kind === "quiz") && cleanOptions.length >= 2) {
      const { error: optionError } = await supabase.from("activity_options").insert(cleanOptions.map((label, position) => ({ activity_id: activity.id, label, position })));
